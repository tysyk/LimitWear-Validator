from __future__ import annotations

from core.config import APPAREL_CONFIDENCE_THRESHOLD


def run(ctx) -> None:
    if not isinstance(ctx.ml, dict):
        ctx.ml = {}

    if not isinstance(ctx.scene, dict):
        ctx.scene = {}

    try:
        from ml.apparel.inference_apparel import predict_apparel

        result = predict_apparel(ctx.bgr)

        confidence = float(result.get("confidence", 0.0) or 0.0)
        label = str(result.get("label", "unknown"))

        is_reliable = confidence >= APPAREL_CONFIDENCE_THRESHOLD
        is_apparel = label == "apparel" and is_reliable

        ctx.ml["apparel"] = {
            **result,
            "isReliable": is_reliable,
            "threshold": APPAREL_CONFIDENCE_THRESHOLD,
            "source": "ml",
        }

        ctx.scene["is_apparel"] = is_apparel
        ctx.scene["apparel_confidence"] = round(confidence, 4)
        ctx.scene["apparel_label"] = label
        ctx.scene["apparel_source"] = "ml"

        ctx.set_debug_section(
            "ml_apparel",
            {
                "label": label,
                "confidence": round(confidence, 4),
                "isReliable": is_reliable,
            },
        )

    except Exception as exc:
        ctx.ml["apparel"] = {
            "label": "unknown",
            "confidence": None,
            "isReliable": False,
            "threshold": APPAREL_CONFIDENCE_THRESHOLD,
            "source": "unavailable",
            "error": str(exc),
        }

        ctx.scene["apparel_source"] = "scene_fallback"

        ctx.add_warning(
            "ML apparel classifier is unavailable, so scene heuristics will be used as fallback."
        )
        ctx.add_error("ml_apparel", str(exc), critical=False)

        ctx.set_debug_section(
            "ml_apparel",
            {
                "label": "unknown",
                "confidence": None,
                "isReliable": False,
                "error": str(exc),
            },
        )

    ctx.mark_step_done("ml_apparel")